...
export function ImageCarousel() {
    ...
    return (
        <Carousel withIndicators height={'100%'} w={'100%'} p={'md'} nextControlIcon={<IconArrowRight size={20} color='black' className='shadow-xl'/>} previousControlIcon={<IconArrowLeft size={20} color='black' className='shadow-xl'/>}>
            {flatImages && flatImages.map((image, index) => (
                <Carousel.Slide key={index} className='flex flex-col items-center justify-center h-full w-full'>
                    <ImageFormat image={image} imSize={imSize} index={index} />
                    <p className='text-sm md:text-base text-center'>
                        {flatTitles[index]}
                    </p>
                    <p className='text-xs md:text-sm text-center'>{`${index + 1} of ${flatImages && flatImages.length}`}</p>
                </Carousel.Slide>
            ))}
        </Carousel>
    )
