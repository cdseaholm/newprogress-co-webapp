...
const PanelTwo = ({handleClickedTab, panelKey}: {handleClickedTab: (index: number) => void, panelKey: number}) => {
    ...
    return (
        <div className="flex flex-col justify-center items-center border border-themeStone rounded-md p-2 w-full h-full" key={panelKey}>
            ...
            <p>
                {`Given my pricing structure,`} <a className="text-themeBlue hover:underline hover:text-themeStone cursor-pointer" onClick={() => handleClickedTab(1)}>see here,</a> {`I find that most of my clients are looking for a long term relationship with their developer. So, site maintenance options make the most sense with monthly payment. HOWEVER, this doesn't mean it can't apply to up front contracts. Some companies and people may need something cleaned up quickly, or there is an issue with their hosting or current site. I am more than happy to discuss what a solution looks like if this is the case. In the rare off chance that my`} <span className="underline font-semibold">original source code</span> {`contains bugs or errors, I will fix this for free. That being a said, I am a perfectionist and I don't forsee that being an issue.`}
            </p>
        </div>
    );
}
        ...
        <div className="flex flex-col justify-start items-center border border-themeStone rounded-md p-2 w-full text-center space-y-2" key={panelKey}>
            ...
            <p>
                {`My preferred hosting provider is `} 
                    <a className="text-themeBlue hover:underline hover:text-themeStone cursor-pointer" onClick={() => openInNewTab('https://www.vercel.com/docs')}>
                        Vercel.
                    </a>
            </p>
            <p>
